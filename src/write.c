...
//         type = 0;
    ...
    for (i = 0; i < c->chunk_item->num_stripes; i++) {
        ...
        if (stripe->device->devobj->Flags & DO_BUFFERED_IO) {
            ...
            stripe->Irp->MdlAddress = IoAllocateMdl(data, length, FALSE, FALSE, NULL);
        ...
        IrpSp->Parameters.Write.Length = length;
        IrpSp->Parameters.Write.ByteOffset.QuadPart = address - c->offset + cis[i].offset;
        ...
        if (offset.QuadPart > start_data || offset.QuadPart + *length < end_data) {
            if (changed_length) {
                if (fcb->inode_item.st_size > start_data) 
                    Status = read_file_fcb(fcb, data + bufhead, start_data, fcb->inode_item.st_size - start_data, NULL);
                else
                    Status = STATUS_SUCCESS;
            } else
                Status = read_file_fcb(fcb, data + bufhead, start_data, end_data - start_data, NULL);
