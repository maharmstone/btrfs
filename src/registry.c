...
NTSTATUS registry_load_volume_options(device_extension* Vcb) {
    ...
    options->max_inline = min(mount_max_inline, Vcb->superblock.node_size - sizeof(struct btrfs_header) - sizeof(leaf_node) - sizeof(EXTENT_DATA) + 1);
    ...
    do {
        ...
        if (NT_SUCCESS(Status)) {
            ...
            if (FsRtlAreNamesEqual(&ignoreus, &us, true, NULL) && kvfi->DataOffset > 0 && kvfi->DataLength > 0 && kvfi->Type == REG_DWORD) {
                ...
                options->max_inline = min(*val, Vcb->superblock.node_size - sizeof(struct btrfs_header) - sizeof(leaf_node) - sizeof(EXTENT_DATA) + 1);
