...
static void log_file_checksum_error_shared(device_extension* Vcb, UINT64 treeaddr, UINT64 addr, UINT64 devid, UINT64 extent) {
    ...
    Status = read_data(Vcb, treeaddr, Vcb->superblock.node_size, NULL, TRUE, (UINT8*)tree, NULL, NULL, NULL, FALSE, 0, FALSE, 0);
    if (!NT_SUCCESS(Status)) {
...
static void log_tree_checksum_error_shared(device_extension* Vcb, UINT64 offset, UINT64 address, UINT64 devid) {
    ...
    Status = read_data(Vcb, offset, Vcb->superblock.node_size, NULL, TRUE, (UINT8*)tree, NULL, NULL, NULL, FALSE, 0, FALSE, 0);
    if (!NT_SUCCESS(Status)) {
        ERR("read_data returned %08x\n", Status);
        goto end;
    }
    ...
    if (c->chunk_item->num_stripes > 1) {
            ...
            for (i = 0; i < c->chunk_item->num_stripes; i++) {
                if (context->stripes[i].csum_error && !c->devices[i]->readonly) {
                    Status = write_data_phys(c->devices[i]->devobj, cis[i].offset + offset - c->offset,
                                             context->stripes[good_stripe].buf, context->stripes[i].length, FALSE);
        ...
        for (i = 0; i < c->chunk_item->num_stripes; i++) {
            if (!c->devices[i]->readonly) {
                Status = write_data_phys(c->devices[i]->devobj, cis[i].offset + offset - c->offset,
                                         context->stripes[i].buf, context->stripes[i].length, FALSE);
                if (!NT_SUCCESS(Status)) {
                    ERR("write_data_phys returned %08x\n", Status);
                    return Status;
                }
                        ...
                        if (recovered) {
                            ...
                            if (!c->devices[j + k]->readonly) {
                                ...
                                Status = write_data_phys(c->devices[j + k]->devobj, cis[j + k].offset + offset - c->offset,
                                                         context->stripes[j + goodstripe].buf, context->stripes[j + goodstripe].length, FALSE);
                ...
                if (recovered) {
                    ...
                    for (k = 0; k < sub_stripes; k++) {
                        if (!c->devices[j + k]->readonly) {
                            ...
                            Status = write_data_phys(c->devices[j + k]->devobj, cis[j + k].offset + offset - c->offset,
                                                        context->stripes[j + k].buf, context->stripes[j + k].length, FALSE);
    ...
    for (i = 0; i < sectors_per_stripe; i++) {
        ...
        if (num_errors == 0) { // parity error
            ...
            if (RtlCheckBit(&context->is_tree, bad_off1)) {
                ...
                if ((crc32a == *((UINT32*)th1->csum) && th1->address == addr) || (crc32b == *((UINT32*)th2->csum) && th2->address == addr)) {
                    if (crc32a != *((UINT32*)th1->csum) || th1->address != addr) {
                        ...
                        if (crc32b != *((UINT32*)th2->csum) || th2->address != addr) {
                            // fix parity 2
                            stripe = parity1 == 0 ? (c->chunk_item->num_stripes - 1) : (parity1 - 1);
                ...
                if (crc32a == context->csum[bad_off1] || crc32b == context->csum[bad_off1]) {
                    if (crc32a != context->csum[bad_off1]) {
                        RtlCopyMemory(&context->stripes[bad_stripe1].buf[(num * c->chunk_item->stripe_length) + (i * Vcb->superblock.sector_size)],
                        ...
                        if (crc32b != context->csum[bad_off1]) {
                            // fix parity 2
                            stripe = parity1 == 0 ? (c->chunk_item->num_stripes - 1) : (parity1 - 1);
    ...
    do {
        ...
        if (c->chunk_item->type & BLOCK_FLAG_RAID6) {
            for (i = 0; i < read_stripes; i++) {
                scrub_raid6_stripe(Vcb, c, &context, stripe_start, stripe, i);
            }
        } else {
            for (i = 0; i < read_stripes; i++) {
            ...
            if (context.stripes[i].Irp) {
                ...
                if (context.stripes[i].rewrite) {
                    Status = write_data_phys(c->devices[i]->devobj, cis[i].offset + context.stripes[i].offset,
                                             context.stripes[i].buf, read_stripes * c->chunk_item->stripe_length, FALSE);
